...
function buildClassificationList($classifications){ 
  ...
  function buildVehiclesDisplay($vehicles){
   ...
   foreach ($vehicles as $vehicle) {
    $dv .= '<li>';
    $dv .= "<a href='/phpmotors/vehicles/?action=vehicleDetails&invId=$vehicle[invId]'><img src='$vehicle[imgPath]' alt='Pic of $vehicle[invMake] $vehicle[invModel]></a>";
   //  $dv .= '<div>';
   //  $dv .= '<hr>';
   //  $dv .= '</div>';
    $dv .= "<h2><a href='/phpmotors/vehicles/?action=vehicleDetails&invId=$vehicle[invId]'>$vehicle[invMake] $vehicle[invModel]</a></h2>";
    $dv .= '<br>';
  ...
  function buildVehicleInfo($vehicle) {
   $dv = '<div id="inv-details">';
       $dv .= "<div id='inv-image'>";
       $dv .= "<img id='vehicles' src='$vehicle[imgPath]' alt='Pic of $vehicle[invMake] $vehicle[invModel]'>";
           $invPrice = $vehicle['invPrice'];
           $invPrice = number_format($invPrice,2,'.',',');
           $dv .= "<p>Price: &#36;$invPrice</p>";
       $dv .= "</div>";
...
function showThumbnails($vehicleThumbs) {
    $tn = '<div id="thumbnails">';
    $tn .= '<h3 class="show-mobile">Thumbnail Images</h3>';
    $tn .= '<ul id="thumbs">';

  foreach($vehicleThumbs as $thumb) {
    $tn .= "<li class='list-thumbnails'>";
    $tn .= "<img src='$thumb[imgPath]' alt='A nice pic of $thumb[imgName] shown in a small view'>";
    $tn .= "</li>";
  }
